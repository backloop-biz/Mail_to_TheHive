...
api = TheHiveApi('http://127.0.0.1:9000', 'wtKi9NOpA+yTSPgv0HbDvTp3ANtKfc4u')
...
#                        message='test'
#                        )
#response = api.create_case_observable('8', domain)
#if response.status_code == 201:
#    print(json.dumps(response.json(), indent=4, sort_keys=True))
#    print('')
#else:
...
response = api.create_case_observable(sys.argv[1], file_observable)
if response.status_code == 201:
    print(json.dumps(response.json(), indent=4, sort_keys=True))
    print('')
    id = response.json()['id']
